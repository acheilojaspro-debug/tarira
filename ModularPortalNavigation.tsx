  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 bg-[#172554] border-b border-blue-900/80 shadow-[0_8px_30px_rgba(30,58,138,0.28)] px-2 sm:px-4 xl:px-6 py-1 sm:py-1.5 transition-colors duration-300 text-white">
        ...
        <div className="flex items-center justify-between text-[11px] pb-1 border-b border-blue-800/60 w-full overflow-visible relative">
          <div className="flex items-center gap-1.5 sm:gap-2 truncate text-white">
            ...
            <span className="hidden md:inline text-blue-100 font-sans font-medium tracking-wide truncate">
              {currentLang === 'pt' ? 'Ecossistema Corporativo de Moçambique' : 'Mozambique Corporate Ecosystem'}
            </span>
          </div>

          {/* Right side: Units Dropdown (deslocado para o topo para evitar qualquer sobreposição com o botão Entrar) & Indicador */}
          <div className="flex items-center gap-1.5 sm:gap-2 font-sans shrink-0">
            {/* 🌟 BOTAO UNIDADES DO ECOSSISTEMA - ZONA TOP UTILITY */}
            <div className="relative">
              <button
                id="top-nav-units-dropdown-btn"
                type="button"
                onClick={() => {
                ...
                className={`px-2.5 py-0.5 rounded-full text-[10px] sm:text-[11px] font-mono font-bold uppercase tracking-wider transition-all cursor-pointer flex items-center gap-1.5 border shrink-0 whitespace-nowrap ${
                  isSolutionsDropdownOpen
                    ? 'bg-white text-[#172554] border-white shadow-md'
                    : 'text-white hover:text-white bg-white/15 hover:bg-white/25 border-white/25 shadow-xs'
                }`}
                title="Unidades Especializadas do Ecossistema TARIRA"
              >
                <Layers className={`w-3 h-3 ${isSolutionsDropdownOpen ? 'text-[#172554]' : 'text-blue-300'}`} />
                <span>{currentLang === 'pt' ? 'Unidades' : 'Units'}</span>
                <ChevronDown className={`w-2.5 h-2.5 transition-transform ${isSolutionsDropdownOpen ? 'rotate-180 text-[#172554]' : 'text-white'}`} />
              </button>

              {isSolutionsDropdownOpen && (
                <div 
                  className="absolute top-full right-0 mt-2 w-80 bg-white border border-border rounded-2xl shadow-[0_20px_50px_rgba(15,23,42,0.25)] p-2.5 z-50 animate-fade-up text-slate-800"
                ...
                >
                  ...
                  <div className="space-y-1 pt-1.5">
                    {[
                      { id: 'connect', name: 'TARIRA Connect', desc: 'Técnicos de campo & manutenção', badge: 'Técnicos', badgeColor: 'bg-blue-50 text-[#172554] border-blue-200', icon: <TariraConnectIcon size={18} className="text-[#172554]" /> },
            ...
            <span className="hidden sm:flex items-center gap-1 text-[9px] sm:text-[10px] text-white font-mono px-1.5 sm:px-2 py-0.5 rounded-full bg-blue-950/60 border border-blue-800/80">
        ...
        <div className="flex items-center justify-between gap-1 sm:gap-2 xl:gap-3 pt-1 w-full min-w-0">
          {/* Logo - Marca e Identidade Visual: Ícone + TARIRA no Mobile (compacto e limpo), com Slogans completos a partir de tablet/desktop */}
          <div 
          ...
          >
            ...
            <div className="block sm:hidden">
              <TariraLogo lightBg={false} size="xs" align="left" withSlogan={false} />
            </div>
          </div>
          ...
          <nav className="hidden lg:flex items-center gap-0.5 xl:gap-1.5 shrink min-w-0">
            <button
              ...
              onClick={() => setActiveTab('landing')}
              className={`px-2 xl:px-3 py-1.5 rounded-full text-[11px] xl:text-xs font-black uppercase tracking-wider transition-all cursor-pointer shrink-0 whitespace-nowrap ${
                activeTab === 'landing'
            ...
            <button
              ...
              className={`px-2 xl:px-3 py-1.5 rounded-full text-[11px] xl:text-xs font-black uppercase tracking-wider transition-all cursor-pointer flex items-center gap-1.5 shrink-0 whitespace-nowrap ${
                activeTab === 'services'
                  ? 'bg-white text-[#172554] shadow-md font-black'
                  : 'text-white hover:text-white hover:bg-white/15 font-bold'
              }`}
            ...
            {/* Vagas & Candidaturas Button - Sem badge Novo */}
            <button
          ...
          <div className="flex items-center gap-1 sm:gap-2 xl:gap-2.5 shrink-0 justify-end ml-auto">
            <form 
              onSubmit={(e) => {
                e.preventDefault();
            ...
            {/* 🌟 AUTHENTICATION / ACCOUNT PORTAL CONTROLS */}
            ...
            {isUserAuthenticated ? (
              ...
              <div className="flex items-center gap-1 sm:gap-2 shrink-0 z-10">
                ...
                <button
                  ...
                  className="flex items-center gap-1 !text-[#172554] text-[#172554] font-black py-1 sm:py-1.5 px-2 sm:px-4 rounded-full bg-white hover:bg-blue-50 shadow-md transition-all text-xs cursor-pointer shrink-0 whitespace-nowrap border border-white hover:scale-[1.02] active:scale-95"
                  title="Criar Nova Conta no Portal TARIRA"
                >
                  <UserPlus className="w-3.5 h-3.5 !text-[#172554] text-[#172554] stroke-[2.5] shrink-0" />
                  <span className="!text-[#172554] text-[#172554] font-black hidden xs:inline">{currentLang === 'pt' ? 'Criar Conta' : 'Sign Up'}</span>
                  <span className="!text-[#172554] text-[#172554] font-black xs:hidden">{currentLang === 'pt' ? 'Registo' : 'Sign Up'}</span>
                </button>
              </div>
            )}
            </div>
            ...
            <button
              ...
              className="lg:hidden flex items-center justify-center p-1.5 sm:p-2 rounded-xl bg-white text-[#172554] font-black border border-white shadow-md active:scale-95 transition-all cursor-pointer shrink-0 min-w-[36px] min-h-[36px] sm:min-w-[40px] sm:min-h-[40px] z-20 ml-0.5"
              title={isMobileMenuOpen ? 'Fechar Menu' : 'Abrir Menu (Três Pontinhos)'}
              aria-label="Abrir Menu de Três Pontinhos"
            >
              {isMobileMenuOpen ? (
                <X className="w-5 h-5 text-[#172554] stroke-[3]" />
              ) : (
                <MoreVertical className="w-5 h-5 text-[#172554] stroke-[3]" />
              )}
            </button>
          </div>
        </div>
