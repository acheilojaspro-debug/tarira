...
async function hydrateStateFromSupabase() {
  ...
  try {
    const [c, cl, h, p, rs, cr, sa, br, ci, al, pr, cp, deletedIds, op, deletedOpIds, deletedClIds, jv, ja, deletedJvIds, jvFallback] = await Promise.all([
      ...
      loadTable<JobVacancyState>("job_vacancies"),
      loadTable<any>("job_applications"),
      getContentValue<string[]>("deleted_job_vacancy_ids"),
      getContentValue<JobVacancyState[]>("job_vacancies_fallback"),
    ]);
    ...
    if (deletedJvIds && Array.isArray(deletedJvIds)) {
      deletedJvIds.forEach(id => deletedJobVacancyIds.add(id));
    }
    const loadedVacancies = (jv && jv.length ? jv : (jvFallback || []));
    if (loadedVacancies.length > 0) {
      const cleanVacancies = loadedVacancies.filter(v => !deletedJobVacancyIds.has(v.id));
      jobVacancies.length = 0;
      jobVacancies.push(...cleanVacancies);
    } else if (deletedJobVacancyIds.size > 0) {
      // Admin excluded mockups/vacancies, keep clean
      jobVacancies.length = 0;
    } else {
      jobVacancies.length = 0;
      jobVacancies.push(...DEFAULT_SEED_VACANCIES.filter(s => !deletedJobVacancyIds.has(s.id)));
    }
    if (ja && ja.length) {
      jobApplications.length = 0;
      jobApplications.push(...ja);
    }
...
// Zerado por diretriz estrita de integridade: sem vagas fictícias ou mockups.
// Vagas começam vazias e recebem apenas vagas reais criadas pelo administrador ou sincronizadas com o Supabase.
const DEFAULT_SEED_VACANCIES: JobVacancyState[] = [];

const jobVacancies: JobVacancyState[] = [];
const deletedJobVacancyIds = new Set<string>([
...
// Candidaturas a Vagas & Contratos Estruturados
app.get("/api/job-applications", (req, res) => {
  noCache(res);
  const email = (req.query.email as string)?.toLowerCase();
  if (email) {
    return res.json(jobApplications.filter(a => a.email && a.email.toLowerCase() === email));
  }
...
app.post("/api/job-applications", (req, res) => {
  ...
  const fullApp = {
    ...
    status: application.status || "Em Triagem"
  };
  jobApplications.unshift(fullApp);
  res.json({ success: true, application: fullApp });
});

// ══════════════════════════════════════════════════════════════════════════
// VAGAS & CONTRATOS TÉCNICOS (CRUD COMPLETO & SINCRONIZAÇÃO SUPABASE)
// ══════════════════════════════════════════════════════════════════════════

app.get("/api/job-vacancies", (req, res) => {
  noCache(res);
  const includeMockups = req.query.include_mockups !== "false";
  let list = jobVacancies.filter(v => !deletedJobVacancyIds.has(v.id));
  if (!includeMockups) {
    list = list.filter(v => !v.isMockup);
  }
  res.json(list);
});

app.post("/api/job-vacancies", (req, res) => {
  const {
    title,
    category,
    department,
  ...
  const newVacancy: JobVacancyState = {
    id: req.body.id || `vaga-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`,
    title: String(title).trim(),
    category: category === 'oficios' ? 'oficios' : 'quadros',
    department: String(department).trim(),
    location: String(location || 'Maputo Cidade').trim(),
    province: province || 'Maputo Cidade',
    type: String(type || 'Tempo Inteiro').trim(),
    contractDuration: contractDuration || (category === 'oficios' ? 'Contrato Técnico de Curta, Média e Longa Duração' : 'Contrato Permanente'),
    isLongTermTechnicalContract: Boolean(isLongTermTechnicalContract),
    status: status || 'sourcing',
    statusLabel: statusLabel || (status === 'screening' ? 'Em Triagem' : status === 'upcoming' ? 'Abertura Iminente' : 'Sourcing Ativo'),
    summary: String(summary || '').trim(),
    requirements: Array.isArray(requirements) 
      ? requirements.map((r: any) => String(r).trim()).filter(Boolean)
      : (typeof requirements === 'string' ? requirements.split('\n').map((s: string) => s.trim()).filter(Boolean) : []),
    expectedOpenings: String(expectedOpenings || 'Vagas em aberto').trim(),
    isMockup: Boolean(isMockup),
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString()
  };

  jobVacancies.unshift(newVacancy);
  setContentValue("job_vacancies_fallback", jobVacancies).catch(() => {});
  
  auditLogs.unshift({
    id: `log-${Date.now()}`,
    type: "VAGA_CRIADA",
    detail: `Nova vaga publicada: ${newVacancy.title} (${newVacancy.category.toUpperCase()})`,
    timestamp: "Agora"
  });
...
app.put("/api/job-vacancies/:id", (req, res) => {
  const { id } = req.params;
  const idx = jobVacancies.findIndex(v => v.id === id);
  if (idx === -1) {
    return res.status(404).json({ error: "Vaga não encontrada" });
  }

  const existing = jobVacancies[idx];
  const reqs = req.body.requirements !== undefined
    ? (Array.isArray(req.body.requirements) 
        ? req.body.requirements.map((r: any) => String(r).trim()).filter(Boolean)
        : (typeof req.body.requirements === 'string' ? req.body.requirements.split('\n').map((s: string) => s.trim()).filter(Boolean) : existing.requirements))
    : existing.requirements;

  const updated: JobVacancyState = {
    ...existing,
    ...req.body,
    id: existing.id,
    isMockup: req.body.isMockup !== undefined ? Boolean(req.body.isMockup) : existing.isMockup,
    requirements: reqs,
    updatedAt: new Date().toISOString()
  };

  jobVacancies[idx] = updated;
  setContentValue("job_vacancies_fallback", jobVacancies).catch(() => {});
  
  auditLogs.unshift({
    id: `log-${Date.now()}`,
    type: "VAGA_ATUALIZADA",
    detail: `Vaga atualizada: ${updated.title}`,
    timestamp: "Agora"
  });
...
app.delete("/api/job-vacancies/:id", async (req, res) => {
  const { id } = req.params;
  const idx = jobVacancies.findIndex(v => v.id === id);
  const found = idx !== -1 ? jobVacancies[idx] : null;

  if (idx !== -1) {
    jobVacancies.splice(idx, 1);
  }
  deletedJobVacancyIds.add(id);

  // Excluir também do Supabase de imediato
  await deleteFromTable("job_vacancies", id).catch(() => {});
  setContentValue("deleted_job_vacancy_ids", Array.from(deletedJobVacancyIds)).catch(() => {});
  setContentValue("job_vacancies_fallback", jobVacancies).catch(() => {});

  auditLogs.unshift({
    id: `log-${Date.now()}`,
    type: "VAGA_ELIMINADA",
    detail: `Vaga removida: ${found?.title || id}`,
    timestamp: "Agora"
  });
...
// Exclusão definitiva de todas as vagas fictícias / mockups
app.post("/api/job-vacancies/clear-mockups", async (req, res) => {
  const defaultMockupIds = [
    'tech-dev', 'tech-clima', 'finance-audit', 'elec-industrial', 
    'ops-logistics', 'civil-plumbing', 'commercial-b2b', 'hr-talent', 
    'solar-renewable', 'eng-civil-nampula', 'electromech-tete', 'agri-manica'
  ];

  const mockupsToRemove = jobVacancies.filter(v => v.isMockup || defaultMockupIds.includes(v.id));
  const removedCount = mockupsToRemove.length;

  mockupsToRemove.forEach(m => {
    deletedJobVacancyIds.add(m.id);
    deleteFromTable("job_vacancies", m.id).catch(() => {});
  });

  const remaining = jobVacancies.filter(v => !deletedJobVacancyIds.has(v.id));
  jobVacancies.length = 0;
  jobVacancies.push(...remaining);

  await setContentValue("deleted_job_vacancy_ids", Array.from(deletedJobVacancyIds)).catch(() => {});
  await setContentValue("job_vacancies_fallback", jobVacancies).catch(() => {});

  auditLogs.unshift({
    id: `log-${Date.now()}`,
    type: "EXCLUSAO_VAGAS_FICTICIAS",
    detail: `${removedCount} vagas fictícias/mockups eliminadas do sistema para receção de vagas reais.`,
    timestamp: "Agora"
  });
  ...
  res.json({ 
    success: true, 
    removedCount, 
    remainingCount: jobVacancies.length, 
...
async function startServer() {
  ...
  // Serve public directory assets (og-image.png, sitemap.xml, robots.txt, icons)
