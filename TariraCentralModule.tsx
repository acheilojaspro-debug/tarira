...
import { TariraJobVacanciesManager } from './TariraJobVacanciesManager';
...
interface TariraCentralModuleProps {
  ...
  onAddJobVacancy?: (vacancy: Partial<JobVacancy>) => Promise<any>;
  onUpdateJobVacancy?: (id: string, updates: Partial<JobVacancy>) => Promise<any>;
  onDeleteJobVacancy?: (id: string) => Promise<any>;
  onClearMockupVacancies?: () => Promise<any>;
  jobApplications?: any[];
...
export const TariraCentralModule: React.FC<TariraCentralModuleProps> = ({
  ...
  const navTabs: { id: CentralSubTabType; label: string; icon: string; badge?: number; highlight?: boolean }[] = [
    ...
    { id: 'job_vacancies', label: 'Portal de Vagas & Carreiras (CRUD)', icon: '💼', badge: jobVacancies.length, highlight: true },
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ════════════════ SUBTAB: PORTAL DE VAGAS & CARREIRAS (CRUD) ════════════════ */}
      {activeSubTab === 'job_vacancies' && (
        <TariraJobVacanciesManager
          vacancies={jobVacancies}
          onAddVacancy={onAddJobVacancy || (async () => {})}
          onUpdateVacancy={onUpdateJobVacancy || (async () => {})}
          onDeleteVacancy={onDeleteJobVacancy || (async () => {})}
          onClearMockups={onClearMockupVacancies || (async () => {})}
          jobApplicationsCount={jobApplications.length}
          onNavigateToPublicJobs={onNavigateToPublicJobs}
